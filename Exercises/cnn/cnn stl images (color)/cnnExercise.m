...
more off;
imageDim = 64;         % image width or height
...
visibleSize = patchDim * patchDim * imageChannels;  % number of input units or visibleSize = 8 * 8 * 3 = 192
outputSize = visibleSize;   % number of output units
hiddenSize = 400;           % number of hidden units. This is the input to softmax layer
...
% above 3 matrices are available in STL10Features.mat
...
disp('computing convolved and pooled features');
% tic();
...
% fprintf('time taken to compute pooling %f secs', toc());
...
% lSize = numel(pooledFeaturesTrain) / numTrainImages;
% softmaxModel = softmaxTrain(softmaxX, softmaxY,lSize, numClasses, softmaxLambda, options)
