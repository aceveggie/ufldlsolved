...
% 
num_instances = size(data, 2);
hidden = sigmoid(bsxfun(@plus, W1 * data, b1));
% Sparsity constraint
rho = mean(hidden, 2);
recons_data = bsxfun(@plus, W2 * hidden, b2);
cost = sum(vec((data - recons_data).^2)) / 2 / num_instances;
weight_cost = lambda / 2 * (norm(W1, 'fro')^2 + norm(W2, 'fro')^2);
sparsity_cost = beta * sum(sparsityParam * log(sparsityParam ./ rho) + ...
                (1-sparsityParam) * log((1-sparsityParam) ./ (1-rho)));
cost = cost + weight_cost + sparsity_cost;
% cost = cost + beta * sparsity_cost; 
% Backpropagation with sparsity constraint

% OLD % errorterm = (recons_data-data);
% OLD % deltaterm = errorterm .* recons_data .* (1-recons_data);

deltaterm = (recons_data - data);
W2grad = deltaterm * hidden' / num_instances + lambda * W2;
b2grad = sum(deltaterm, 2) / num_instances;

sum_deltaterm2 = W2' * deltaterm;
sparsity_deltaterm2 = beta * (-sparsityParam ./ rho + (1-sparsityParam) ./ (1-rho));
% deltaterm2 = (W2' * deltaterm) .* hidden .* (1-hidden);
deltaterm2 = bsxfun(@plus, sum_deltaterm2, sparsity_deltaterm2) .* hidden .* (1-hidden);
W1grad = deltaterm2 * data' / num_instances + lambda * W1;
b1grad = sum(deltaterm2, 2) / num_instances;
...
% fprintf('sum(z2): %f\n', sum(sum(z2)));
% fprintf('meanrho: %f\n', sum(meanrho));
% fprintf('squareErrorTerm: %f\n', squareErrorTerm);
% fprintf('weightDecayTerm: %f\n', weightDecayTerm);
% fprintf('sparsityPenaltyTerm: %f\n', sparsityPenaltyTerm);
% fprintf('cost: %f\n',cost);
% fprintf('sum(sum(delta2)): %f\n',sum(sum(delta2)));
% fprintf('gradient: %f\n', sum(grad));
