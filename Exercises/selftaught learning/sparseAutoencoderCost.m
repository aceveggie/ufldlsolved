...
[m, n]=size(data);
% feed forward into hidden layer
z1= W1*data + repmat(b1,1,n);
z1 = sigmoid(z1);
% feed forward out of hidden layer
z2 = W2*z1 + repmat(b2, 1, n);
z2 = sigmoid(z2);

meanrho = sum(z1,2)./n; % rho cap

%%%%%%%%%%%% now calculate cost
% now ideally z2 has to be equal to data
simpleCost = sum((z2-data).^2);
% meansquare error
squareErrorTerm = 0.5*sum(simpleCost)/n;
% lambda for regularlization
weightDecayTerm = (1/2)*lambda*(sum(sum(W1.^2)) + sum(sum(W2.^2)));
% KL divergence to measure how different are two distributions
sparsityPenaltyTerm = sparsityParam.*log(sparsityParam./meanrho)+(1-sparsityParam).*log((1-sparsityParam)./(1-meanrho));
sparsityPenaltyTerm = beta*sum(sparsityPenaltyTerm);

% cost contains 3 terms, square error, weight decay term, sparsity term
cost = squareErrorTerm + weightDecayTerm + sparsityPenaltyTerm;
% backpropogation

% delta2
delta2 = - (data - z2) .* (z2 .* (1 - z2));
...
delta1 = (W2')*delta2;
sparext = repmat(calcSpar,1,n);
delta1 = delta1 + sparext;
delta1 = delta1.*(z1.*(1-z1)) ;
...
W2grad = W2grad./n + (W2.*lambda);
b2grad = sum(delta2,2)./n;
...
W1grad = W1grad./n + (W1.*lambda);
b1grad = sum(delta1,2)./n;
