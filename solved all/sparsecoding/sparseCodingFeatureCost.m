...
errorMatrix = weightMatrix*featureMatrix-patches;
cost = norm(errorMatrix, 'fro')^2/numPatches + ...
    lambda*sum(vec(sqrt(groupMatrix*(featureMatrix.^2) + epsilon)));
grad = 2*(weightMatrix'*weightMatrix*featureMatrix - weightMatrix'*patches)/numPatches;
grad = grad + lambda*groupMatrix'*(groupMatrix*(featureMatrix.^2)+epsilon).^(-0.5) .* featureMatrix;
grad = grad(:);
end